...
\subtitle{Lecture 2 (Part 1) --- Climate meets the economy: the DICE/CDICE model, solved with Deep Equilibrium Nets}
...
\vspace{0.2cm}
{\footnotesize \emphc{Connection to Part 2 of this lecture:} once the model can be solved fast and reliably, GP surrogates and Bayesian active learning turn the SCC into a \emph{distribution} over uncertain parameters.}
\end{frame}
...
With effects unfolding over centuries, $\rho$ and $\psi$ dominate the size of the SCC --- one reason global sensitivity analysis (Part 2 of this lecture, and Lecture 3) matters so much in climate economics.
\end{block}
\end{columns}
\end{frame}
...
% --- Slide 25: DEQN recap I: the idea ---
\begin{frame}{Recap from Lecture 1, I: The DEQN Idea}
\small
\textbf{The problem.} A dynamic model pins down an unknown policy $p: X \to Y \subset \R^K$ only \emph{implicitly}, through its first-order equilibrium conditions:
\[
\mathbf{G}(\x, p(\x)) = \bm{0} \qquad \text{for all states } \x \in X .
\]

\vspace{0.1cm}
\textbf{The DEQN answer.} Approximate the policy with a deep neural network, $p(\x) \approx \nn(\x)$, and train it so the equilibrium conditions hold --- \emphc{self-supervised}: the equations replace labeled data. No grid is ever built, so the method stays \emphc{global} and scales to high-dimensional state spaces.
...
\vspace{0.1cm}
{\footnotesize Notation: we write $\bm{\varphi}$ for the network weights (Lecture 1 used $\theta$, which here is reserved for the abatement-cost parameters $\theta_{1,t}, \theta_2$).}
\end{frame}

% --- Slide 25b: DEQN recap II: the training loop ---
\begin{frame}{Recap from Lecture 1, II: The DEQN Training Loop}
\footnotesize
The loss is the squared residual of the equilibrium conditions along a simulated path, minimized by gradient descent on the weights:
\[
...
\begin{block}{One training episode (repeat until the loss stops falling)}
\begin{enumerate}\itemsep1pt
    \item \textbf{Simulate:} generate a path $\x_0 \to \x_1 \to \cdots \to \x_{T_{\text{sim}}}$ using the \emph{current} policy $\nn$ and the model's transition laws;
    \item \textbf{Evaluate:} compute the residuals $G_m(\x_t, \nn(\x_t))$ on mini-batches drawn from the path;
    \item \textbf{Update:} take Adam steps on $\bm{\varphi}$, then \emphc{re-simulate} with the improved policy.
\end{enumerate}
\end{block}

\vspace{0.1cm}
Because training states come from the model's own simulation, the network learns exactly where the economy lives --- the \emphc{ergodic set}. And the loss is interpretable: it measures (squared) \emphc{equilibrium-condition errors}, so a small loss certifies accuracy in economic units.

\vspace{0.1cm}
{\scriptsize Details: Lecture 1 slides (\emph{Deep Equilibrium Nets}) and the Lecture 1 notebooks.}
\end{frame}
...
\column{0.48\textwidth}
\textbf{Looking ahead to Part 2.}
\begin{itemize}
    \item Next: GP surrogates map uncertain parameters to the SCC; Bayesian active learning picks where to solve next.
    \item Payoff: SCC \emphc{distributions} and global sensitivity indices instead of point estimates.
\end{itemize}
\end{columns}
...
\begin{center}
\textbf{Next up:}\\[6pt]
{\large Part 2: surrogate models and\\[2pt]
Gaussian processes}\\[12pt]
{\small Hands-on CDICE in Practical Session 2\\[2pt]
(notebooks \texttt{01}--\texttt{03}); production code:}\\[2pt]
