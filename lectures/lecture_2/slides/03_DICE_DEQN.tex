...
\includegraphics[width=0.88\linewidth]{fig/cod.png}\\[4pt]
\includegraphics[width=0.88\linewidth]{fig/examplefunc.pdf}
\end{columns}
\end{frame}
...
\begin{align*}
    \Omega_t &= \psi_1 T^{\text{AT}}_{t} + \psi_2 \left(T^{\text{AT}}_{t}\right)^2
    && \text{(damages)} \\
    \Theta_{t} &= \theta_{1,t}\, \mu_t^{\theta_2}
    && \text{(abatement cost)} \\
...
\includegraphics[width=0.92\linewidth]{fig/damage.png}
...
Equilibrium masses [GtC] & $M_{\text{EQ}}$ & (607, 489, 1281) \\
Initial masses 2015 [GtC] & $M_{\text{INI}}$ & (851, 628, 1323) \\
Temperature coefficients & $c_1, c_3, c_4$ & 0.137, 0.73, 0.00689 \\
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=0.98\linewidth, height=4.2cm,
...
&& \text{(Euler equation for capital)} \\[4pt]
...
\vspace{0.2cm}
{\footnotesize Architecture and hyperparameters (layers, width, $\alpha^{\text{learn}}$, $N_{\text{path length}}$, $\vartheta$) live in the replication repo: \url{https://github.com/ClimateChangeEcon/Climate_in_Climate_Economics}.}
\end{frame}
...
Loss components & 1 Euler & $N$ Euler $+$ ARC $+$ $N$ FB & 8 (Euler, budget, \emphc{5 shadow prices}, FB) \\
